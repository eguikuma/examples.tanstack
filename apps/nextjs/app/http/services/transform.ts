...
const enhance = async (url: string, name: string) => {
  ...
  const translation = result.success ? japanese(result.data.names) : undefined
...
export const enrich = async (pokemon: { name: string; url: string }) => {
  ...
  const types = await Promise.all(
    response.data.types.map(({ type }: { type: { name: string; url: string } }) =>
      enhance(type.url, type.name),
    ),
  )
