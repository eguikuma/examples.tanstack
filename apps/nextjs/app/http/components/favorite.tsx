...
export const Favorite = ({
  ...
  const add = http.post('/api/favorites', {
    key: ['favorites', 'add'],
    ...
    failure: (data) => {
      console.log('add.failure', data)
    },
  })

  const remove = http.delete(`/api/favorites/${id}`, {
    ...
    failure: (data) => {
      console.log('remove.failure', data)
    },
  })

  const pending = add.pending || remove.pending
  ...
  return (
    ...
    >
      <svg
        ...
        role="img"
      >
        <title>{liked ? 'お気に入りから削除' : 'お気に入りに追加'}</title>
        <path
          strokeLinecap="round"
